...
let _ =
  ...
  let test_vorbis () =
    ...
    if not (Vorbis.Decoder.check_packet packet) then
      raise Not_found;
    fill os;
    let page = Ogg.Sync.read sync in
    Ogg.Stream.put_page os page ;
  ...
  let vdr,cmt = Vorbis.Decoder.comments decoder in
    Printf.printf
      ...
      infos.Vorbis.vorbis_version
      infos.Vorbis.audio_channels
      infos.Vorbis.audio_samplerate;
    Printf.printf "* vendor: %s\n" vdr;
    List.iter (fun (c, v) -> Printf.printf "* %s: %s\n" (String.lowercase c) v) cmt;
    flush_all ();
    (* Using vorbis to decode the ogg. *)
    print_newline ();
    Printf.printf "Decoding stream..\n"; flush_all ();
    let tmpdst,oc = Filename.open_temp_file ~mode:[Open_binary] "stream2wav" ".raw" in
      (
          ...
          try
            ...
            do
              try
                ...
                let buf = Array.map (Array.map (fun x -> int_of_float (x*.32767.))) buf in
      ...
      let oc = open_out_bin !dst in
        ...
        output_short oc infos.Vorbis.audio_channels; (* channels *)
